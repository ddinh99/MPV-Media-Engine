...
- The bridge does not relay MPV's replies/events back over the WebSocket — only app→MPV commands flow. So the Command Log's `MPV: …` lines and anything depending on IPC responses or `observe_property` do not work over the WS path today, only over a genuine direct TCP connection (which, per above, isn't reachable on a real Windows mpv.exe either) or a manual named-pipe test client.
