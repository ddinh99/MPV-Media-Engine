      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              _HelpStep('1', 'Start MPV with IPC enabled:',
                  'mpv --input-ipc-server=localhost:9001 --input-ipc-server-type=tcp movie.mp4'),
              const SizedBox(height: 12),
              _HelpStep('2', 'Set socket path in the connection bar:',
                  'localhost:9001  (default)'),
              const SizedBox(height: 12),
              _HelpStep('3', 'Click Connect — status turns green.', ''),
              const SizedBox(height: 12),
              _HelpStep('4', 'Select a preset or adjust sliders.', 'With Auto-apply ON, changes send instantly to MPV.'),
              const SizedBox(height: 12),
              _HelpStep('5', 'Click "Apply Now" to manually push settings.', ''),
              const SizedBox(height: 12),
              const Divider(),
              const SizedBox(height: 8),
              Text(
                '💡 Tip: Set Windows audio output to 96000 Hz for the smoothest MPV playback.',
